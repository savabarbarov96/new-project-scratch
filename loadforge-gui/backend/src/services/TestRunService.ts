...
import type { TestSpecification } from '../types/index.js';
...
export class TestRunService {
  ...
  async stopTestRun(testRunId: string): Promise<boolean> {
    ...
    console.log(`Attempting to stop test ${testRunId} with status: ${testRun.status}`);

    // Allow stopping tests in various states, not just 'running'
    if (testRun.status === 'completed' || testRun.status === 'failed' || testRun.status === 'cancelled') {
      console.log(`Test ${testRunId} is already in final state: ${testRun.status}`);
      // Clean up any remaining references
      this.activeTestSpecs.delete(testRunId);
      return true; // Consider it successfully "stopped"
    }

    // Try to stop the test in the engine
    const stopped = await this.loadTestEngine.stopTest(testRunId);
    
    if (stopped || testRun.status !== 'running') {
      // Update status to cancelled regardless of previous state
      ...
      console.log(`Test ${testRunId} stopped successfully`);
      return true;
    }

    console.log(`Failed to stop test ${testRunId}`);
    return false;
  }
  ...
  private setupEngineEventHandlers(): void {
    ...
    this.loadTestEngine.on('test-metrics', (testRunId: string, metrics: any) => {
      ...
      if (!specification) {
        console.warn(`No specification found for test ${testRunId}, skipping metrics emission`);
        return;
      }
      
      // Emit WebSocket event for real-time metrics
      if (webSocketService) {
        // Priority 1: Handle individual request metrics (marked with isIndividualRequest or has statusCode+responseTime)
        if (metrics.isIndividualRequest || (metrics.statusCode && metrics.responseTime)) {
          // Individual request log with actual URL and method from specification
          const requestLog = {
            testRunId,
          ...
          console.log(`🟢 Emitting individual request log for test ${testRunId}:`, {
            method: requestLog.method,
            url: requestLog.url,
            statusCode: requestLog.statusCode,
            responseTime: requestLog.responseTime,
            phase: requestLog.phase
          });
          
          webSocketService.emitRequestLog(requestLog);
        }
        // Priority 2: Handle aggregated metrics  
        else if (metrics.responseCount !== undefined) {
          // Aggregated metrics
          const testMetrics = {
            testRunId,
            timestamp: metrics.timestamp || new Date(),
          ...
          console.log(`🔵 Emitting aggregated test metrics for test ${testRunId}:`, {
            responseCount: testMetrics.responseCount,
            currentRps: testMetrics.currentRps,
            phase: testMetrics.phase
          });
          
          webSocketService.emitTestMetrics(testMetrics);
        }
        // Debug: Log unrecognized metrics
        else {
          console.log(`🟡 Received unrecognized metrics format for test ${testRunId}:`, Object.keys(metrics));
        }
      }
    });
