...
export async function testRunRoutes(fastify: FastifyInstance) {
  ...
  fastify.post<TestRunParams>('/tests/:id/stop', async (request: FastifyRequest<TestRunParams>, reply: FastifyReply) => {
    try {
      ...
      const { id } = request.params;
      console.log(`[STOP REQUEST] Attempting to stop test run: ${id}`);
      
      // Validate the test run ID format
      if (!id || id.length !== 24) {
        console.log(`[STOP REQUEST] Invalid test run ID format: ${id}`);
        return reply.status(400).send({
          success: false,
      ...
      // First check if the test run exists and get its current status
      try {
        const testRun = await testRunService.getTestRun(id);
        console.log(`[STOP REQUEST] Test run ${id} found with status: ${testRun.status}`);
        
        // Allow stopping tests in any state, but provide appropriate messaging
        const stopped = await testRunService.stopTestRun(id);
        
        if (stopped) {
          const message = testRun.status === 'running' 
            ? 'Test run stopped successfully'
            : `Test run stopped successfully (was in '${testRun.status}' status)`;
          
          console.log(`[STOP REQUEST] ${message} for test ${id}`);
          return reply.send({
            success: true,
            message,
          } as ApiResponse);
        } else {
          console.log(`[STOP REQUEST] Failed to stop test ${id} - service returned false`);
          return reply.status(400).send({
            success: false,
            error: 'Test could not be stopped - it may have already completed or failed',
          } as ApiResponse);
        }
      } catch (statusError) {
        console.error(`[STOP REQUEST] Error checking/stopping test run ${id}:`, statusError);
        
        if (statusError instanceof Error) {
          if (statusError.message.includes('not found')) {
            return reply.status(404).send({
              success: false,
              error: 'Test run not found - it may have already been deleted',
            } as ApiResponse);
          } else if (statusError.message.includes('Cast to ObjectId failed')) {
            return reply.status(400).send({
              success: false,
              error: 'Invalid test run ID format',
            } as ApiResponse);
        ...
        return reply.status(500).send({
          success: false,
          error: statusError instanceof Error ? statusError.message : 'Failed to stop test run',
        } as ApiResponse);
      }
    } catch (error) {
      console.error(`[STOP REQUEST] Unexpected error stopping test run ${request.params.id}:`, error);
